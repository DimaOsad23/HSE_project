...
class Solution:
            ...
            for p_2 in range(p_1 + 1, len(coords) - 2):
                for p_3 in range(p_2 + 1, len(coords) - 1):
                    for p_4 in range(p_3 + 1, len(coords)):
                        ...
                        if self.false_quadrilateral(coords[p_1], coords[p_2],
                                                    coords[p_3], coords[p_4]):
                            continue
                        # Перебор возможных четырёхугольников с вершинами в данных 4 точках
                        # Сравнение их площади с максимальной
                        self.space_quadrilateral(coords[p_1], coords[p_2],
                                                 coords[p_3], coords[p_4])
                        self.space_quadrilateral(coords[p_1], coords[p_3],
                                                 coords[p_2], coords[p_4])
                        self.space_quadrilateral(coords[p_1], coords[p_4],
                                                 coords[p_2], coords[p_3])
    ...
    def false_quadrilateral(self, p_1, p_2, p_3, p_4):
        ...
        if self.cos_vectors_square(self.vector(p_1, p_2),
                                   self.vector(p_1, p_3)) == 1:
            return True
    ...
    def cos_vectors_square(self, vector_1, vector_2):
        cosin = (vector_1[0] * vector_2[0] + vector_1[1] * vector_2[
            1]) ** 2 / (
    ...
    def space_quadrilateral(self, p_1, p_2, p_3, p_4):
        if self.length_vector(p_1, p_2) * self.length_vector(p_3,
                                                      p_4) * self.sin_vectors(
                self.vector(p_1, p_2), self.vector(p_3, p_4)) / 2 > self.space:
            # Перезаписываетс максимальное значение площади
            self.space = self.length_vector(p_1, p_2) * self.length_vector(p_3,
                                                                           p_4) * self.sin_vectors(
                self.vector(p_1, p_2),
                self.vector(p_3, p_4)) / 2
