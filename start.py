...
class Start:
    ...
    def start(self):
        ...
        self.canvas.bind('<Button-3>',
                         self.wrk.mouse_move_coords_sys_click)
        self.canvas.bind('<B3-Motion>',
                         self.wrk.mouse_move_coords_sys_move)
        self.wrk.coord()
    ...
    def menu_3(self):
        ...
        vst_menu.add_command(label='Клавиатура',
                             command=lambda: self.wrk.keyboard(
                                 'insert'))
        ...
        self.main_menu.add_cascade(label="Редактирование",
                                   menu=file_menu3)
