...
class Work:
    ...
    def __init__(self, root, pn_control, pn_graph, canvas):
        ...
        self.coord_x_y = Label(self.canvas, textvariable=self.x_y,
                               bg="white", fg="black",
                               font=("bold", 10))
    ...
    def coord(self):
        ...
        for i in range(1, summ):
            self.canvas.create_line(i * 640 / summ, 0, i * 640 / summ,
                                    640, fill="light grey", width=1)
            self.canvas.create_line(0, i * 640 / summ, 640,
                                    i * 640 / summ,
        ...
        if self.delta_x <= self.xmin:
            ...
            self.create_text_y(640 / 2 + 15 - 640 * self.delta_x
                               / (2 * self.xmax))
        if self.delta_y <= self.ymin:
            ...
            self.create_text_x(640 / 2 + 10 + 640 * self.delta_y
                               / (2 * self.ymax))
        ...
        for i in range(1, summ):
            if i - self.xmax + self.delta_x != 0:
                self.canvas.create_text(i * 640 / summ, coordinata,
                                        text=i
                                             - self.xmax + self.delta_x,
                                        fill='black')
            elif coordinata > 10:
                self.canvas.create_text(max(320 - self.delta_x * 640
                                            / (
                                                    self.xmax - self.xmin) + 7,
                                            15),
                                        min(320 + self.delta_y * 640
                                            / (
                                                    self.ymax - self.ymin) + 7,
                                            630),
                                        text=0, fill='black')
    ...
    def create_system_coord_x(self):
        summ = -self.xmin + self.xmax
        self.canvas.create_line(0, 320 + 320 * self.delta_y / self.ymax,
                                640,
                                320 + 320 * self.delta_y / self.ymax,
                                fill="black", width=2, arrow=LAST,
                                arrowshape="5 10 5")
        self.canvas.create_text(632,
                                330 + 320 * self.delta_y / self.ymax,
                                text='X', anchor=W, fill="black")

        for i in range(1, summ):
            self.canvas.create_line(i * 640 / summ,
                                    320 + 640 * self.delta_y / (
                                            2 * self.ymax),
                                    i * 640 / summ,
                                    320 + 5 + 640 * self.delta_y / (
                                            2 * self.ymax),
                                    fill="black", width=1)
    ...
    def create_system_coord_y(self):
        ...
        self.canvas.create_line(320 - 320 * self.delta_x / self.xmax, 0,
                                ...
                                width=2, arrow=FIRST,
                                arrowshape="5 10 5")
        self.canvas.create_text(330 - 320 * self.delta_x / self.xmax,
                                10, text='Y', anchor=W, fill="black")
        for i in range(1, summ):
    ...
    def enter_canvas(self, event):
        ...
        self.canvas.bind('<Button-1>', lambda
            event: self.canvas.focus_set())  # Метод, устанавливающий фокус на холст
    ...
    def mouse_move_coords_sys_move(self, event):
        self.delta_mouse_x = self.delta_mouse_x + (
                self.but3_coords_x - event.x) / 640 * (
                                     self.xmax - self.xmin)
        self.delta_mouse_y = self.delta_mouse_y + (
                self.but3_coords_y - event.y) / 640 * (
                                     self.ymax - self.ymin)
        ...
        x = round(
            (self.but3_coords_x - 320) * self.xmax / 320 + self.delta_x,
            1)
        y = round(
            (320 - self.but3_coords_y) * self.ymax / 320 + self.delta_y,
            1)
    ...
    def draw_coords_mode(self):
        ...
        if self.special_arg == 1:
            ...
            if len(self.red_point) != 0:
                self.canvas.create_oval(320 + 320 / self.xmax * (float(
                    self.red_point[0]) - self.delta_x) - 3,
                                        320 - 320 / self.ymax * (float(
                                            self.red_point[
                                                1]) - self.delta_y) - 3,
                                        320 + 320 / self.xmax * (float(
                                            self.red_point[
                                                0]) - self.delta_x) + 3,
                                        320 - 320 / self.ymax * (float(
                                            self.red_point[
    ...
    def start_change_bg_color(self):
        self.clear_all()
        lab = Label(self.pn_control, text='Выберете цвет фона',
                    font='TimesNewRoman 14', bg=self.bg_color)
        lab.place(width=480, x=0, height=40)
        but_blue = Button(self.pn_control, bg='blue',
                          activebackground='blue',
                          command=lambda: self.change_bg_color(
                              'cornflower blue'))
        but_green = Button(self.pn_control, bg='green',
                           activebackground='green',
                           command=lambda: self.change_bg_color(
                               'light green'))
        but_yellow = Button(self.pn_control, bg='yellow',
                            activebackground='yellow',
                            command=lambda: self.change_bg_color(
                                'light goldenrod'))
        but_grey = Button(self.pn_control, bg='grey',
    ...
    def random(self, mode):
        ...
        if mode == 'input':
            ...
            if len(self.coords) == 25:
                showinfo('Информация',
                         'Нельзя добавить точки, так как уже введено максимально допустимое количество.')
            else:
                self.random_range(mode)
    ...
    def random_range(self, mode):
        ...
        lab_range_x = Label(self.pn_control,
                            text='Введите диапазон значений x',
                            font='TimesNewRoman 10',
                            bg=self.bg_color)
        lab_range_y = Label(self.pn_control,
        ...
        ent_range_x.focus()
        but_range = Button(self.pn_control, text='Задать диапазон',
                           font='TimesNewRoman 12',
                           command=lambda: self.quantity_rand_point(
                               [txt_range_x.get(), txt_range_y.get()],
                               mode))
    ...
    def quantity_rand_point(self, range_points, mode):
        try:
            range_random = [round(float(range_points[0].split()[0]), 1),
                            round(float(range_points[0].split()[1]), 1),
                            round(float(range_points[1].split()[0]), 1),
                            round(float(range_points[1].split()[1]), 1)]
            if range_random[1] <= range_random[0] or range_random[3] <= \
                    range_random[2] or range_random[0] <= -100 or \
                    range_random[2] <= -100 or range_random[1] > 100 or \
                    range_random[3] > 100 or \
                    len(range_points[0].split()) > 2 or len(
                range_points[1].split()) > 2:
                return 1 / 0
            self.clear_pn_control()
            txt = StringVar()
            lab = Label(self.pn_control,
            ...
            ent.focus()
            but_range = Button(self.pn_control,
                               text='Задать количество',
                               font='TimesNewRoman 12',
                               command=lambda: self.add_random(txt,
                                                               range_random,
                                                               mode))
    ...
    def add_random(self, txt, range_random, mode):
        try:
            ...
            if (range_random[1] - range_random[0]) * (
                    range_random[3] - range_random[2]) * 100 \
                ...
                showerror('Неверный формат',
                    ...
                    self.coords) + quantity_points > 25 and mode == 'insert':
                showerror('Проблема',
                          'Вы задаёте слишком много точек. Точек должно быть не больше 25.')
                ...
                while k != quantity_points:
                    num = [
                        round(uniform(range_random[0], range_random[1]),
                              1),
                        round(uniform(range_random[2], range_random[3]),
                              1)]
            ...
            showerror('Неверный формат',
                      'Неверный формат. Проверьте, что количество точек указано правильно.')
    ...
    def keyboard_add(self):
        txt_x = StringVar()
        txt_y = StringVar()
        lab_x = Label(self.pn_control, text='Введите значение x',
                      font='TimesNewRoman 10', bg=self.bg_color)
        lab_y = Label(self.pn_control, text='Введите значение y',
                      font='TimesNewRoman 10', bg=self.bg_color)
        ...
        ent_x.focus()
        but_add_point = Button(self.pn_control, text='Добавить точку',
                               font='TimesNewRoman 12',
                               command=lambda: self.add_point(txt_x,
                                                              txt_y,
                                                              ent_x,
                                                              ent_y))
        stop_add_but = Button(self.pn_control,
        ...
        try:
            ...
            if [x_coord, y_coord] in self.coords:
                ...
                showerror('Неверные координаты',
                          'Введены некорректные координаты. Координаты точек должны лежать в диапазоне (-100, 100).')
            else:
                self.coords.append([x_coord, y_coord])
                self.canvas.create_oval(320 + 320 / self.xmax * x_coord,
            ...
            if len(self.coords) == 25:
                self.clear_all()
                showinfo('Ввод завершён',
                         'Ввод завершён, так как введёно максимально допустимое количество точек (25).'
                         )
        except:
            showerror('Неверный формат',
                      'Проверьте правильно ли введены координаты точек.')
    ...
    def mouse_add(self):
        ...
        self.canvas.bind('<Enter>',
                         lambda event: self.canvas.bind('<B3-Motion>',
                                                        self.mouse_move_coords_sys_move))
        self.canvas.bind('<Button-1>', self.click)
        lab = Label(self.pn_control, text='Выберете точку мышкой',
                    font='TimesNewRoman 14', bg=self.bg_color)
    ...
    def click(self, event):
        ...
        if not ([round(x_coord + self.delta_x, 1),
                 round(y_coord + self.delta_y,
                       1)] in self.coords) and abs(
            round(x_coord + self.delta_x, 1)) < 100 and abs(
            round(y_coord + self.delta_y, 1)) < 100:
            self.canvas.create_oval(320 + 320 / self.xmax * x_coord,
                                    320 - 320 / self.ymax * y_coord,
                                    320 + 320 / self.xmax * x_coord,
                                    320 - 320 / self.ymax * y_coord,
                                    width=3)
            self.coords.append([round(x_coord + self.delta_x, 1),
    ...
    def open_file(self, mode):
        try:
            ...
            for coords in f.readlines():  # Создаём список координат без повторений и проверяем эти координаты
                if not [round(float(coords.split()[0]), 1),
                        round(float(coords.split()[1]),
                              1)] in list_coords:
                    list_coords.append(
                        [round(float(coords.split()[0]), 1),
                         round(float(coords.split()[1]), 1)])
                if abs(round(float(coords.split()[0]),
                             1)) >= 100 and abs(
                    round(float(coords.split()[1]), 1)) >= 100:
                    return 1 / 0
            f.close()
            if len(self.coords) + len(
                    list_coords) > 25 and mode == 'insert':
                showerror('Проблема',
            ...
            showerror('Неверные координаты',
                      'Неверные координаты в файле. Координаты должны лежать в диапозоне (-100, 100).')
        except:
            showerror('Неверный формат',
    ...
    def coords_output(self):
        lab_coords = Label(self.pn_control, text='Координаты (x,y):',
                           font='TimesNewRoman 12', bg=self.bg_color)
        ...
        box_coords = Listbox(output_frame, yscrollcommand=y_scroll.set,
                             font='TimesNewRoman 12', bg=self.bg_color,
    ...
    def editing(self):
        ...
        if len(self.coords) == 0:
            ...
            stop_but = Button(self.pn_control,
    ...
    def edit_event(self, event):
        x_coord = round((event.x - 320) * self.xmax / 320, 1)
        y_coord = round((320 - event.y) * self.ymax / 320, 1)
        if [round(x_coord + self.delta_x, 1),
            round(y_coord + self.delta_y, 1)] in self.coords:
            self.canvas.create_oval(320 + 320 / self.xmax * x_coord - 3,
                                    320 - 320 / self.ymax * y_coord - 3,
                                    320 + 320 / self.xmax * x_coord + 3,
                                    320 - 320 / self.ymax * y_coord + 3,
                                    outline='red',
                                    fill='red', width=1)
            self.red_point = [round(x_coord + self.delta_x, 1),
                              round(y_coord + self.delta_y, 1)]
            self.canvas.unbind('<Button-1>')
            self.edit_but()
    ...
    def edit_but(self):
        self.clear_pn_control()
        lab_info = Label(self.pn_control, text='Редактируемая точка',
                         font='TimesNewRoman 12', bg=self.bg_color)
        lab_point = Label(self.pn_control, text='(' + str(
            self.red_point[0]) + ' ,' + str(self.red_point[1]) + ')',
                          font='TimesNewRoman 12', bg=self.bg_color)
        lab_info.place(width=480, x=0, y=10, height=30)
        lab_point.place(width=480, x=0, y=50, height=30)
        del_but = Button(self.pn_control, text='Удалить',
                         font='TimesNewRoman 12',
                         command=self.del_point)
        del_but.place(width=140, x=170, height=40, y=90)
        change_but = Button(self.pn_control, text='Изменить',
                            font='TimesNewRoman 12',
                            command=self.change)
        change_but.place(width=140, x=170, height=40, y=140)
        cancel_but = Button(self.pn_control, text='Отмена',
                            font='TimesNewRoman 12',
    ...
    def del_point(self):
        ...
        if len(self.coords) == 0:
            self.clear_all()
            showinfo('Завершение редактирования',
                     'Редактирование прекращено, так как недостаточно точек (нет точек).')
        else:
            self.editing()
    ...
    def change(self):
        ...
        ent_x.focus()
        but_change_point = Button(self.pn_control, text='Заменить',
                                  font='TimesNewRoman 12',
                                  command=lambda: self.change_point(
                                      txt_x, txt_y, ent_x, ent_y))
    ...
    def moving(self):
        showinfo('Перемещение',
                 'Перемещение по плоскости осуществляется при помощи стрелок клавиатуры или при помощи зажатой ПКМ')
    ...
    def solution_of_task(self):
        ...
        if len(self.answer) == 0:
            showerror('Проблема',
                      'Невозможно построить четырёхугольник по заданным точкам.')
        else:
            self.clear_all()
            self.draw_points()
            self.coords_output()
    ...
    def solution_2_of_task(self):
        self.answer, self.space = self.solution.main(
            convex_hull(self.coords))
        if len(self.answer) == 0:
            showerror('Проблема',
                      'Невозможно построить четырёхугольник по заданным точкам.')
        else:
    ...
    def draw_solution(self, solution):
        self.canvas.create_line(320 + 320 / self.xmax * (
                float(solution[0][0]) - self.delta_x),
                                320 - 320 / self.ymax * (float(
                                    solution[0][1]) - self.delta_y),
                                320 + 320 / self.xmax * (float(
                                    solution[1][0]) - self.delta_x),
                                320 - 320 / self.ymax * (float(
                                    solution[1][1]) - self.delta_y))
        self.canvas.create_line(320 + 320 / self.xmax * (
                float(solution[1][0]) - self.delta_x),
                                320 - 320 / self.ymax * (float(
                                    solution[1][1]) - self.delta_y),
                                320 + 320 / self.xmax * (float(
                                    solution[2][0]) - self.delta_x),
                                320 - 320 / self.ymax * (float(
                                    solution[2][1]) - self.delta_y))
        self.canvas.create_line(320 + 320 / self.xmax * (
                float(solution[2][0]) - self.delta_x),
                                320 - 320 / self.ymax * (float(
                                    solution[2][1]) - self.delta_y),
                                320 + 320 / self.xmax * (float(
                                    solution[3][0]) - self.delta_x),
                                320 - 320 / self.ymax * (float(
                                    solution[3][1]) - self.delta_y))
        self.canvas.create_line(320 + 320 / self.xmax * (
                float(solution[3][0]) - self.delta_x),
                                320 - 320 / self.ymax * (float(
                                    solution[3][1]) - self.delta_y),
                                320 + 320 / self.xmax * (float(
                                    solution[0][0]) - self.delta_x),
                                320 - 320 / self.ymax * (float(
                                    solution[0][1]) - self.delta_y))
        self.special_arg = 2
        self.write_answer()
    ...
    def write_answer(self):
        ...
        text_coords = 'Координаты вершин четырёхугольника:\n' + '(' + str(
            ...
            self.answer[2][0]) + ', ' + str(
            self.answer[2][1]) + ')\n' + '(' + str(
            self.answer[3][0]) + ', ' + str(self.answer[3][1]) + ')\n'

        lab_answer = Label(self.pn_control, text=text_answer,
                           font='TimesNewRoman 14', bg=self.bg_color)
        lab_answer.place(width=480, x=0, height=20, y=215)
        lab_coords = Label(self.pn_control, text=text_coords,
                           font='TimesNewRoman 12', bg=self.bg_color)
        lab_coords.place(width=480, x=0, height=105, y=250)
        lab_space = Label(self.pn_control,
