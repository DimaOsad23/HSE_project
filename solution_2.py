...
class Solution_2:
    ...
    def __init__(self):
        ...
        self.answer = (
            []
        )  # Переменная хранящая, последовательность точек, которая соответствует максимальной "площади"
    ...
    def check_quadrilaterals(self, coords):
        if not (self.false_quadrilateral(
            ...
            self.space_quadrilateral(
                coords[0], coords[3], coords[1], coords[2])
            q_list = self.answer + []
            return q_list
        return coords
    ...
    def cos_vectors_square(self, vector_1, vector_2):
        ...
        if (
            cosin > 1
        ):  # Эта проверка нужна из-за способа хранения вещественных чисел языком Python
            cosin = 1
        return cosin
